...
local function _tabulate(tab, depth, cycle, width)
   cycle = cycle or {}
   depth = depth or 0
   if type(tab) ~= "table" then
      ts_coro(tab, nil, width)
      return nil
   end
   if depth > C.depth or cycle[tab] then
      ts_coro(tab, "tab_name", width)
      return nil
   end
   cycle[tab] = true
   ...
   if _M then
      ...
      ts_coro(tab, "mt", width)
      yield(c.base(" = "), 3)
      _tabulate(_M, depth + 1, cycle, width)
   end
   ...
   for j, key in ipairs(keys) do
      if is_array then
         _tabulate(key, depth + 1, cycle, width)
      else
         val = tab[key]
         if type(key) == "string" and key:find("^[%a_][%a%d_]*$") then
            ts_coro(key, nil, width)
            yield(c.base(" = "), 3)
         else
            yield(c.base("["), 1)
               -- we want names or hashes for any lvalue table,
               -- 100 triggers this
            _tabulate(key, 100, cycle, width)
            yield(c.base("] = "), 4)
         end
         _tabulate(val, depth + 1, cycle, width)
      end
   end
   yield(C_BRACE(), 1, "end")
   return nil
...
local function lineGen(tab, depth, cycle, disp_width)
   ...
   return function()
      ...
      while phrase.yielding do
         local line, len, event = iter(tab, depth, cycle, disp_width)
         if line == nil then
...
local function c_data(value, str, width)
   local meta = reflect.getmetatable(value)
   yield(str, #str)
   if meta then
      yield(c.base " = ", 3)
      ts_coro(meta, nil, width)
   end
end
...
ts_coro = function (value, hint, width)
