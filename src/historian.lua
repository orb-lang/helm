...
local function _highlight(line, frag, best)
   local hl = {}
   while #frag > 0 do
      local char
      ...
      if not best and #frag <= 1 then
         ...
         Color = color.search_hl
      end
      hl[#hl + 1] = color.base(line:sub(1, at -1))
      hl[#hl + 1] = Color(char)
      line = line:sub(at + 1)
   end
   hl[#hl + 1] = color.base(line)
   return concat(hl):gsub("\n", color.stresc("\\n"))
end

local function _collect_repr(collection)
   local i = 1
   local first = true
   return function()
      if #collection == 0 then
         if first then
            first = false
            return color.alert "No results found"
         else
            return nil
         end
      end
      ...
      local next_line = alt_seq
                        .. _highlight(v, collection.frag, collection.best)
