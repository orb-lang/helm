...
tuple of `(input, ordinal)`, since we'll be recording every meaningful action
in order\.
...
upon\.  We'll also include a third table, `run_map`, with `(run, key, value)`,
...
\#NB
a draft as a point for further discussion\.
