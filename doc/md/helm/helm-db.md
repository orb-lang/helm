...
- \#Todo:
  ...
  - [ ]  Some of these tables should be strict\.  Perhaps all?  Decide\.
...
A line is any unique string entered by a user at some point in time\.
...
#### round
  ...
  Some lines, but not all, constitute an action: the canonical example is
entering some Lua into helm, producing, typically, some results\.

A line with associated responses is called a round\.

Migration 7 will have three sorts of responses which a line can have, and this
may not be a complete set, but it will always be the case that there is *one*
response, and one line, per round\.
...
point to `round`, but this gives us none\-to\-many responses, while our data
model permits exactly one\.
...
than one of these from existing, with some sort of post\-hook, so that we
don't have a round with both an error and a result, which is also
`'pending'`\.
...
Run actions are every important thing which happens within a run\.

If the action is a round of input, the action points to the riff round of that
input, and \.class is `IN`\.  Actions of any other class have a null
`riff_round`, while serving as pointers into the run for other tables, notably
the `run_riff` table\.
