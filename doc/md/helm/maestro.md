...
\#todo
for itself, throwing the rest to Modeselektor, but I'm not seeing a guiding
principle for what those should be, other than "things Maestro **can** process",
but that's apparently not a good implementation? Also, the same logic ends up
being duplicated on Modeselektor\.
...
\#todo
and it deserves some thought as to whether this should be the 
normal
 way
to quit helm\.
