...
### Session:loadID\(name\_or\_index\)

Determines the session ID based on either a name, or the index within the list
of sessions for the project\. Also retrieves the accepted state and, if needed,
the session title\.
...
function Session.loadID(session, name_or_index)
   local index = tonumber(name_or_index)
   if index and floor(index) == index then
      ...
      if #results < index then
         io.stderr:write(format('Cannot load session #%d, only %d available.\n', index, #results))
         os.exit(1)
      end
