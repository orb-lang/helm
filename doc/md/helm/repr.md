...
```
### make_token(str, color[, event[, quote]])

Assembles a "token" structure from the given string, color value,
and optional event name. If ``quote`` is truthy, also wraps the string
in quotation marks. The structure looks like:
...
Handles conversion of nonprinting characters and quotation marks into their
escaped forms, with the ``escapes`` property indicating which characters
...
local function make_token(str, color, event, quote)
...
local function oneLine(phrase, long)
   ...
   while true do
      ...
      if _disp(line) >= phrase.width then
         ...
         local rest
         if token.total_disp < MIN_SPLIT_WIDTH then
            rest = token
...
ts_coro = function(value, hint, phrase)
   ...
   elseif typica == "string" then
      ...
      -- If no quoting needed, fall through to the common yield at the bottom
