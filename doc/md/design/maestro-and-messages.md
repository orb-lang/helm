...
cleared manually.  This doesn't really express that the flag is _consumed_ by
...
whenever we need one.  A message can't _just_ be array parameters and ``.n``,
...
Note that _every_ field in the above is optional, because a reply is also a
...
- reply:  A flag.  When set to ``true``, the Actor receiving the message is
          ...
          Note that this is _a_ mechanism, not _the_ mechanism, for handling
...
figures that out, and we don't really have to _tell_ Zoneherd to reply, it's
...
So yes, we _can_ handle these cases with custom logic, and probably _should_
