...
parameters, greater than or equal to zero, and return an _instance_ of Widget.
...
there's no _one thing_ which has the Widget-nature already, so why keep adding
nouns?
...
We describe any value accessed with a dot as a _field_, and if the value lives
on the table itself, rather than being returned through ``__index``, it is also
a _slot_, so ``instance.field``.  Any valued accessed with a colon is a _method_,
and must be callable, so a function or a table with a ``__call`` metamethod: we
call this _passing a message_, so ``instance:message(...)``.
...
``function Widget.method(widget, ...)``, although the _section header_ will say
``*** Widget:method(...)``.
...
The first thing is that an actor is _sui generis_, and unique.  Some are
singletons, some are not: but there must be some distinctive aspect of the
actor which gives it personality.


Second is that an actor **acts**, that is, it does things.  It doesn't exist
(only) to be acted upon, but to take action; and it doesn't exist primarily to
make other objects (see, not a useless word!), although many of them do
...
A given grammar is also not an actor. Not really. They are _sui generis_, but
...
in response to _events_, which are currently all created by input.  This is
...
sequence of input, and ``modeS`` responds to that sequence by _selecting_ the
response on the basis of the _mode_ which helm is in at that exact instant.
...
actor is the _Boss_ of that actor.  We can be as whimsical as we want in
...
Are they _sui generis_?  Yes.  Do they have personality, do they do things to
...
``__repr`` (=SelectionList=) plus a ``Resbuf`` for suggestions and search results.
(Funny thing, these _are_ actually "results" in the general sense, but, not
the same kind as the primary usage of ``Resbuf``, namely REPL evaluation
results.)  We _might_ need to specialize this into ``Searchbuf`` and
...
And the Rainbuf we have _is a buffer_, so it has a good name.  It needs to be
